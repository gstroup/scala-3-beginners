...
object HOFsCurrying {
  ...
  //  a HOF uses functions as either arguments or result
  val aHof: (Int, (Int => Int)) => Int = (x, func) => x + 1
  val anotherHof: Int => (Int => Int) = x => (y => y + 2 * x)

  // quick exercise
  val superfunction: (Int, (String, (Int => Boolean)) => Int) => (Int => Int) = (x, func) => y => x + y + 1
  // a function that takes 2 args.  first is an int, second is a function
  //  returns a function
  //  start with outermost parens and arrow.
  //= (x, func) => (y => x + y)

  // examples: map, flatMap, filter
  //  they all take other functions as args
  ...
  val tenThousand = nTimes(plusOne, 10000, 2)
  ...
  def nTimes_v2(f: Int => Int, n: Int): Int => Int =
    // This function is recursive, not tail recursive.  it can cause a StackOverflowError!!
    if (n <= 0) (x: Int) => x
...
//  val superAdder_v2 = toCurry[Int, Int, Int](_ + _) // same as superAdder
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("*** " + toCurry[Int, Int, Int](_ + _)(31)(8))
    println("*** " + toCurry[String, String, String](_ + _)("hello ")("world"))
